...
description: A photography-first interface that turns marketing into a museum gallery — rendered in Apple's signature dark mode. Edge-to-edge product tiles alternate soft-black and elevated-black canvases, framed by SF Pro Display headlines with negative letter-spacing and a single Action Blue (#0a84ff) interactive color. UI chrome recedes so the product can speak — no decorative gradients, no shadows on chrome, only the one signature drop-shadow under product imagery resting on a surface.
...
colors:
  ...
  surface-pearl: "#1c1c1e"
  surface-tile-1: "#1c1c1e"
  surface-tile-2: "#2c2c2e"
  surface-tile-3: "#0d0d0d"
  surface-black: "#000000"
  surface-chip-translucent: "rgba(118, 118, 128, 0.24)"
  on-primary: "#ffffff"
  on-dark: "#f5f5f7"
...
Apple's web presence in dark mode is a masterclass in **reverent product photography framed by near-invisible UI on a black canvas**. Every page is a stack of edge-to-edge product "tiles" — alternating soft-black and elevated-black canvases, each centered on a hero headline, a one-line tagline, two tiny blue pill CTAs, and an impossibly crisp product render. Nothing competes with the product. Typography is confident but quiet; color is either pure black, a softer off-black, or a one-step-lighter elevated tile; interactive elements are a single, glowing blue.

Density is unusually low even by contemporary SaaS standards. Each tile occupies roughly one viewport, and there is no decorative chrome — no borders, no gradients, no decorative frames, no shadows on headlines. Elevation appears only when a product image rests on a surface (a single soft `rgba(0, 0, 0, 0.45) 3px 5px 30px` drop for visual weight against the dark canvas). The result is a catalog that feels more like a museum gallery at night: the wall disappears and the artifact takes over, lit from within.

Store and shop surfaces retain the same chassis but switch modes. The product configurator (iPhone 17 Pro, accessories grid) introduces a tight grid of soft-black utility cards at `{rounded.lg}` (18px) radius with a hairline border at `rgba(255, 255, 255, 0.08)`, paired with a persistent thin sub-nav strip. The environment page leans deeper into pure black and more editorial. Across all five surfaces the typographic system, spacing rhythm, and the single blue accent are consistent — this is one design language expressed at different volumes.

**Key Characteristics:**

- Photography-first presentation; UI recedes so the product can speak.
- Alternating full-bleed tile sections: pure black ↔ soft-black ↔ elevated tile, with the color change itself acting as the section divider.
- Single blue accent (`{colors.primary}` — #0a84ff) carries every interactive element. No second brand color exists. (Dark mode brightens Action Blue from #0066cc to #0a84ff so it stays vivid against the black canvas.)
- Two button grammars: tiny blue pill CTAs (`{rounded.pill}`) and compact utility rects (`{rounded.sm}`).
- SF Pro Display + SF Pro Text — negative letter-spacing at display sizes for the signature "Apple tight" headline feel.
- Whisper-soft elevation used only when a product image needs to breathe — exactly one drop-shadow in the entire system.
- Tight two-row nav: slim `{component.global-nav}` + product-specific `{component.sub-nav-frosted}` with persistent right-aligned primary CTA.
- Section rhythm across multiple pages: soft-black hero → elevated product tile → soft-black utility tile → elevated tile → black footer — a predictable pulse anchored in the dark canvas.
...
- **Action Blue** (`{colors.primary}` — #0a84ff): The single brand-level interactive color in dark mode. All text links, all blue pill CTAs ("Learn more", "Buy"), and the focus ring root. This is Apple's iOS-systemBlue-on-dark — brighter than the light-mode Action Blue (#0066cc) to stay vivid against the black canvas. Press state shifts via the active scale transform, not a hex change.
- **Focus Blue** (`{colors.primary-focus}` — #409cff): A marginally brighter sibling of Action Blue, reserved for the keyboard focus ring on buttons (`outline: 2px solid`).
- **Sky Link Blue** (`{colors.primary-on-dark}` — #2997ff): A slightly desaturated blue used for inline body links inside copy blocks — where Action Blue would feel too loud at small sizes.

### Surface

- **Pure Black** (`{colors.canvas}` — #000000): The dominant canvas in dark mode. Page body, global nav bar, edge-to-edge photographic overlays, video player backgrounds. The Apple TV / iOS OLED-friendly true black.
- **Soft Black** (`{colors.canvas-parchment}` — #1c1c1e): The signature Apple dark-mode off-black. Used for alternating soft tiles, footer region, default card surface, and the standard page canvas in store utility sections. One step lighter than pure black — just different enough to create rhythm. (This is iOS systemGray6 in dark mode.)
- **Pearl Surface** (`{colors.surface-pearl}` — #1c1c1e): The fill for default cards and secondary "ghost" buttons — sits at the same elevation as the soft-black canvas, separated only by a hairline border.
- **Soft Tile 1** (`{colors.surface-tile-1}` — #1c1c1e): The primary tile surface on the homepage product grid (matches canvas-parchment).
- **Elevated Tile 2** (`{colors.surface-tile-2}` — #2c2c2e): A micro-step lighter — used where a tile sits directly above or below Tile 1 to create the faintest "raised" separation. (iOS systemGray5 dark.)
- **Recessed Tile 3** (`{colors.surface-tile-3}` — #0d0d0d): A micro-step darker than the soft-black canvas — used at the bottom of the stack and in embedded video/player frames where additional sink is needed.
- **True Black** (`{colors.surface-black}` — #000000): Same as the canvas — reserved for the global nav bar background and contexts where the absolute void is needed.
- **Translucent Chip Gray** (`{colors.surface-chip-translucent}` — rgba(118, 118, 128, 0.24)): The translucent gray chip used over photography for circular control buttons. Apple's iOS dark systemFill — a soft gray that floats above any backdrop.

### Text

- **Light Ink** (`{colors.ink}` — #f5f5f7): The voice of every headline, every body paragraph, and the light utility button's fill on dark mode. Chosen instead of pure white to keep the page feeling photographic rather than fluorescent.
- **Body** (`{colors.body}` — #f5f5f7): Same hex as ink — Apple uses one soft-white tone for all primary text on dark surfaces.
- **Body On Dark** (`{colors.body-on-dark}` — #f5f5f7): Identical to body in dark mode — there is no "light surface" exception anymore; the whole system is dark.
- **Body Muted** (`{colors.body-muted}` — #a1a1a6): Secondary copy where soft-white would be too loud. (iOS secondaryLabel on dark.)
- **Ink Muted 80** (`{colors.ink-muted-80}` — #a1a1a6): Body text on Pearl Surfaces — slightly softer than full light ink.
- **Ink Muted 48** (`{colors.ink-muted-48}` — #636366): Disabled button text and legal fine-print. (iOS tertiaryLabel on dark.)

### Hairlines & Borders

- **Divider Soft** (`{colors.divider-soft}` — rgba(255, 255, 255, 0.06)): The "border" tone on secondary buttons and card edges — functions as a soft glow at the boundary rather than a hard line. The dark-mode equivalent of the light-mode `rgba(0, 0, 0, 0.04)` hairline.
- **Hairline** (`{colors.hairline}` — #2c2c2e): The 1px hairline border on store utility cards and configurator chips. Same hex as Elevated Tile 2 — borders and elevated surfaces share tones in dark mode so cards read as soft pools rather than ringed boxes.

### Brand Gradient

**No decorative gradients.** Atmospheric depth on product photography (the iPhone 17 Pro camera plate, the Apple Watch bands, AirPods reflections) is inherent to the imagery, not a CSS gradient overlay. The environment page's hero uses photographic atmosphere (mountain vista at dusk) but no gradient tokens are defined. Apple is the rare luxury-brand site with zero gradient-based design tokens — in dark mode the temptation to add a "glow" gradient is explicitly avoided.
...
| Soft hairline  | 1px `rgba(255, 255, 255, 0.08)` border             | Utility cards, sub-nav frosted-glass separator                              |
| Backdrop blur  | `backdrop-filter: blur(N)` on Soft Black 80%       | Sub-nav and the iPhone buy floating sticky bar                              |
| Product shadow | `rgba(0, 0, 0, 0.45) 3px 5px 30px 0`               | Product renders resting on a surface (the only true "shadow" in the system) |
...
- Use `{colors.primary}` (Action Blue #0a84ff) for every interactive element — links, pill CTAs, focus signals — and nothing else. The single accent is non-negotiable.
...
- Apply the single product-shadow (`rgba(0, 0, 0, 0.45) 3px 5px 30px`) only to product renders resting on a surface — never on cards, buttons, or text.
...
- Don't slip back into the light-mode Action Blue (#0066cc) — at this contrast level against the black canvas it reads as muddy navy. Dark mode requires #0a84ff.
...
6. The single drop-shadow (`rgba(0, 0, 0, 0.45) 3px 5px 30px`) is reserved for product photography only.
7. When in doubt about emphasis: alternate surface (soft-black → elevated tile) before adding chrome.
